...
public:
    ...
    constant_medium(std::shared_ptr<hittable> boundary, float density, const vec3& albedo)
        : boundary(boundary), neg_inv_density(-1 / density),
    ...
    bool hit(const ray& r, interval ray_t, hit_record& rec) const override 
    {
        ...
        if (!boundary->hit(r, interval(rec1.t + 0.0001f, infinity), rec2)) { return false; }
        ...
        float hit_distance = neg_inv_density * std::logf(random_float());
